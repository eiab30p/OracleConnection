...
class MigrationGraph(object):
    ...
    def apply(self, db, dry_run=False, is_fake=False, name_start=None, name_end=None,
        ...
        # I think if NOT MIGRATION TABLE THEN WE WILL CREATE IT JUST IN CASE
        assert not 'Migration' in db.entities
        # Get DB information and create 
        migration_db = reconstruct_db(db)
        # Will create MIGRATION TABLE if not created already?
        make_migration_entity(migration_db)
        #Simple Connection like before to check and create tables in ORACLE.PY
        migration_db.generate_mapping(create_tables=True, check_tables=True)
        # Tell to end connection
        ...
        with orm.db_session:
            # Migration is the entitiy name when creating for items in db_session
            # We are looking at which entry is in Migration
        ...
        # CHecking if anything needs to be applied
        if not migrations_to_apply:
            print('\nAll migrations are applied.')
            return
        # DOing like we did in Migration
        prev_db = reconstruct_db(db)
